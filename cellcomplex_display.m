...
%if ~isfield(S,'flows') || isempty(S.flows)
    p = graph_undirected_plot(Nodes, Edges, [], edgecolor);
%else
    p = graph_directed_plot(Nodes, Edges, [], edgecolor);
%end
...
P = cellcomplex_compute_facepatches(S, Nodes);                  

% display face polygons --------------------
cellcomplex_display_facepatches(P);                             
...
% =====================================================================
function P = cellcomplex_compute_facepatches(S, Nodes)
% CELLCOMPLEX_COMPUTE_FACEPATCHES  Compute face polygons and signs only.
%
% Output
%   P.xy   : cell array of polygon coordinates
%   P.sign : [F x 1] orientation sign (+1 or -1)
%
% (C) 2026 Moo K. Chung
...
% ============================================================
% Case (B): cycles given as edge lists
% ============================================================
if isfield(S.faces,'cycles') && ~isempty(S.faces.cycles) && iscell(S.faces.cycles)
    ...
    for f = 1:F
        ...
        P.xy{end+1,1}   = xy; %#ok<AGROW>
        P.sign(end+1,1) = fs; %#ok<AGROW>
    end

% ============================================================
% Case (A): faces given as vertex lists
% ============================================================
elseif isfield(S.faces,'vertices') && ~isempty(S.faces.vertices)
    ...
    for f = 1:F
        ...
        P.xy{end+1,1}   = xy; %#ok<AGROW>
        P.sign(end+1,1) = fsall(f); %#ok<AGROW>
    end
...
% =====================================================================
function cellcomplex_display_facepatches(P)
% CELLCOMPLEX_DISPLAY_FACEPATCHES  Display precomputed face polygons.
%
% Input
%   P.xy   : cell array of polygon coordinates
%   P.sign : [F x 1] orientation sign
%
% (C) 2026 Moo K. Chung
